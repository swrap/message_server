...
messageInput.keydown(function(e) {
    if (e.keyCode == 16 && e.keyCode == 13) {
        if(DEBUG) { console.log('shift+enter'); }
        return true;
    }
    if (e.keyCode == 13) {
...
var webOnMessage = function(e) {
    ...
    switch (active_json[ACTION]) {
        ...
        case POST_CONVERSATIONS:
                ...
                if(DEBUG) { console.log("Added conversations."); }
            break;
        case POST_MESSAGES:
                if(DEBUG) { console.log(active_json[THREAD_ID]); }
        ...
        case POST_DATA:
                //received data call
                if(DEBUG) { console.log("POST DATA! [" + active_json[PART_ID] + "] order [" + active_json[ORDER] + "]"); }
...
var webOnErrorClose = function(){
        ...
        setTimeout(function(){
            if(DEBUG) { console.log("Starting loop"); }
...
$('#rt_sendBtn').click(function(){
    ...
    if (rm_message_input.html().length > 0 && convo_id_val != "") {
        ...
        if(DEBUG) { console.log("Temp id message [" + temp_id + "]"); }
        ...
        if(DEBUG) { console.log(messageObjectString); }
        //append to ui
        webSocketCon.send(messageObjectString);
...
function sendConnected() {
    ...
    if(DEBUG) { console.log("Sending Connected"); }
}
...
$(window).focus(function() {
    ...
    if(DEBUG) { console.log("In focus."); }
});

$(window).blur(function() {
    inFocus = 0;
    if(DEBUG) { console.log("Out of focus."); }
});
